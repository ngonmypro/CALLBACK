                            ...
                            <div class="col-4 m-0 py-0 px-2">
                                @if ( isset($data->status) )
                                    @if ($data->status === 'ACTIVE')
                                        <button id="suspend" type="button" class="w-100 btn btn-warning" data-status="{{ $data->status }}">{{__('userManagement.suspend')}}</button>
                                    @else
                                        <button id="suspend" type="button" class="w-100 btn btn-warning" data-status="{{ $data->status }}">{{__('userManagement.unsuspend')}}</button>
                                    @endif
                                @endif
                            </div>
                            <div class="col-4 m-0 py-0 px-2">
                                <a href="{{ URL::to('UserManagement/Edit',$code)}}" class="w-100 btn btn-secondary">{{__('userManagement.edit')}}</a>
                            </div>
                        </div>
                    </div>
                </div>
                                    ...
                                    <div class="p-2 flex-fill w-50">
                                        ...
                                        <div class="py-3 px-5">
                                            ...
                                            @if ( isset($data->status) && !blank($data->status) )
                                                ...
                                                @else
                                                    <span class="badge badge-warning">{{ ucwords($data->status) }}</span>
                                                @endif
                                            @endif
                                        </div>
                        ...
                        success: async function(response) {
                            ...
                            if (response.success) {
                                ...
                                if (formValues) {
                                    ...
                                    formData.append('user_id', `{{ isset($data->code) ? $data->code : '' }}`)
                                    $('#input-wrapper select, #input-wrapper textarea').each(function() {
                                        formData.append($(this).attr('name'), $(this).val())
                                    })
                                    $.ajax({
                                        method: 'post',
                                        url: `{{ URL::to('UserManagement/Request/Suspend') }}`,
                                        data: formData,
                                        processData: false,
                                        contentType: false,
                                        success: function (response) {
                    ...
                    Swal.fire({
                        ...
                        if (!!isConfirm.value) {
                            ...
                            $.ajax({
                                ...
                                data:{
                                    _token : `{{ csrf_token() }}`,
                                    user_id : `{{ isset($data->code) ? $data->code : '' }}`
                                },
                                success: function (response) {
                                    console.log('response: ', response)
                                    $.unblockUI()
