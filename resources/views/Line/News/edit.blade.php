...
<div class="col-12">
    <form action="{{ url('/Line/News/Update') }}" method="POST" enctype="multipart/form-data" id="form-edit-news">
    {{ csrf_field() }}
                    ...
                    <h4 class="mb-3 py-3 card-header-with-border">
                      Edit News
                        <div class="float-right">
                            <button id="save" type="button" class="btn btn-primary text-uppercase" onclick="UpdateSave()">SAVE</button>
                        </div>
                        <input id="news_code" type="hidden" value="{{$news_code}}" name="news_code">
                    </h4>
                </div>
            </div>
                    ...
                    <div class="row">
                      ...
                      <div class="col-12">
                          <input type="text" name="effective_date" id="effective_date" class="form-control" autocomplete="off"
                                 readonly="readonly" value="{{ isset($news_detail->publish_date) ? $news_detail->publish_date : '-' }}" placeholder="Select date"/>
                      </div>
            ...
            <div class="w-50">
                <div class="form-group">
                    <div class="row">
                        ...
                        <div class="col-12">
                            <input type="text" id="title" name="title" value="{{ isset($news_detail->title) ? $news_detail->title : '-' }}" placeholder="" class="form-control required-checked">
                        </div>
                    </div>
                </div>
            </div>
            <div class="w-50">
                <div class="form-group">
                    <div class="row">
                        ...
                        @if($news_detail->title_image != '')
                            <a id="clear" class="clear_img" href="#" onclick="remove_image()" style="display: inline;">&#215;</a>
                            <img style="width: 100%;" id="output" src="{{ $news_detail->title_image }}">
                        @else
...
<script type="text/javascript">
    ...
    function UpdateSave(){
        var delta_content = editor.getContents();
        var json_delta_content = JSON.stringify(delta_content);
        $('#form-edit-news').append('<input type="hidden" id="content_delta" name="content_delta" value="">');
        $('#content_delta').val(json_delta_content);
        $('#form-edit-news').submit();
    }
